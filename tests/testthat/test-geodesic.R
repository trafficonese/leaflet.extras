...
berlin <- c(52.51, 13.4); losangeles <- c(34.05, -118.24); santiago <- c(-33.44, -70.71)
...
test_that("Geodesic", {
  ...
  expect_identical(ts$x$calls[[length(ts$x$calls)]]$args[[1]][[1]][[1]][[1]],
  ...
  expect_identical(ts$x$calls[[length(ts$x$calls)]]$args[[5]]$iconUrl,
                   c("https://leafletjs.com/examples/custom-icons/leaf-red.png",
                     "https://leafletjs.com/examples/custom-icons/leaf-green.png"))
  ...
  ts <- leaflet(cities_df) %>%
    ...
    addGeodesicPolylines(data = sflines, layerId = ~paste0("ID_",id),
      weight = 7, color = ~color, showStats = TRUE,
      markerOptions = markerOptions(draggable = TRUE, title = "some special Title"),
      statsFunction = JS("function(stats) {
                                           return('<h4>Custom Stats Info</h4>' +
